...
train_data = train_data.loc[1000:1200, :]
...
class MLPCustomModel(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None):
        ...
        # Add MLP custom layers
        sequence_output = self.dropout(outputs[1][-1])  # outputs[1][-1]=last hidden state
...
if Resume:
    ...
    model_file_name = f"model_xlmro_{head}_{max_acc}.pt"
...
for epoch in range(num_epochs):
    ...
    if acc > max_acc:
        ...
        torch.save(model.state_dict(), f"model_xlmro_{head}_{acc}.pt")
